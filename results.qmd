...
![Iteration 116 (best). R² = 0.869, 10 outliers. `seed=77` + `sub_diff=6` + `aug=5500` yields the tightest clustering around the diagonal.](figures/rank_50/rate_constants_best.png){#fig-k-best}
...
![Iteration 116 (best, seed=77, sub_diff=6, aug=5500). Same layout. The GNN captures the dominant oscillatory patterns but misses fine-grained temporal structure. The residual reveals the missing homeostatic signal.](figures/rank_50/kinograph_best.png){#fig-kinograph-best}
...
![Iteration 116 (best R²). MLP$_{\text{sub}}$: $\alpha$ at $|s|=1$ is 0.93 — close to the true scale. MLP$_{\text{node}}$: still flat at zero across all 136 iterations — homeostasis not learned.](figures/rank_50/mlp_best.png){#fig-mlp-best}
