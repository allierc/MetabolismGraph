...
**44 iterations completed** across 4 blocks (11 batches). The exploration is ongoing.
...
### R² Trajectory Across 44 Iterations

```{python}
#| code-fold: true
#| label: fig-iterations
#| fig-cap: "Raw R² (bars) and outlier count (dots) across 44 iterations, colored by block. The k_floor breakthrough at iteration 14 lifted R² from 0.07 to 0.51, longer training pushed it to 0.69 by iteration 21, and lr_sub=0.001 broke the plateau to R²=0.726 at iteration 35. Block 4 revealed seed sensitivity (±0.2 R²)."

import matplotlib.pyplot as plt
import numpy as np

iters = list(range(1, 45))
r2 = [
    ...
    # Block 4 (Iter 37-44): Seed sensitivity
...
ax1.axhline(y=0.726, color='#e67e22', linestyle='--', alpha=0.4, label='Peak R² = 0.726')
...
ax1.text(40.5, 1.0, 'Block 4', ha='center', fontsize=9, color='#9b59b6')
...
ax1.annotate('aug=4000', xy=(21, 0.690), xytext=(21, 0.82),
            arrowprops=dict(arrowstyle='->', color='black'), fontsize=7, ha='center', color='black')
ax1.annotate('lr_sub=0.001\npeak', xy=(35, 0.726), xytext=(35, 0.88),
            arrowprops=dict(arrowstyle='->', color='black'), fontsize=7, ha='center', color='black')
ax1.annotate('seed=123\n↓0.24', xy=(41, 0.487), xytext=(41, 0.30),
            arrowprops=dict(arrowstyle='->', color='black'), fontsize=7, ha='center', color='black')

ax1.set_xlabel('Iteration')
ax1.set_ylabel('rate_constants R² (raw)')
ax1.set_ylim(-0.02, 1.05)
ax1.set_xticks([1, 6, 12, 14, 21, 24, 32, 35, 36, 40, 44])
...
ax1.set_title('UCB Exploration: Rate Constant Recovery (44 iterations)')
plt.tight_layout()
plt.show()
```
...
**Best result — Iteration 35**: `lr_sub=0.001`, `data_augmentation_loop=4000`, `coeff_k_floor=1.0`, `coeff_MLP_sub_norm=1.0`
...
*Note: This peak R² is partly seed-dependent (±0.2 variance — see Block 4). The stable baseline is R² ≈ 0.69 (e.g. iteration 42: `aug=4500`, $\alpha = 0.94$).*
...
![Iteration 35 (best). R² = 0.726 (trimmed: 0.975), 15 outliers. Combining `k_floor`, `sub_norm`, `aug=4000`, and `lr_sub=0.001` yields the tightest clustering around the diagonal.](figures/rank_50/rate_constants_best.png){#fig-k-best}

:::

### Kinograph — Best Run (Iteration 35)

![Kinograph montage for iteration 35. Top-left: ground-truth $dc/dt$; top-right: GNN prediction; bottom-left: residual (same color scale as GT); bottom-right: predicted vs. true $dc/dt$ scatter (Pearson = 0.35). The GNN captures the dominant oscillatory patterns but misses fine-grained temporal structure.](figures/rank_50/kinograph_best.png){#fig-kinograph-best}
...
![Iteration 35 (best R²). MLP$_{\text{sub}}$: $\alpha$ at $|s|=1$ is 0.83 — closer to the true scale. MLP$_{\text{node}}$: still flat at zero across all 44 iterations — homeostasis not learned.](figures/rank_50/mlp_best.png){#fig-mlp-best}
...
![UCB tree after 24 iterations (snapshot). Each node represents a hyperparameter configuration; color encodes R² (green = high, red = low). The tree shows how the exploration branched from the `k_floor=1.0` breakthrough (node 14) and converged on the `aug=4000` regime. The tree has since grown to 44 nodes; the overall best is node 35 (R² = 0.726).](figures/rank_50/ucb_tree.png){#fig-ucb-tree}
