...
![MLP functions at iteration 1 (baseline). MLP$_{\text{sub}}$: $\alpha$ at $|s|=1$ is 0.42 — the substrate function is under-scaled. MLP$_{\text{node}}$: flat at zero, contributing nothing to the dynamics.](figures/rank_50/mlp_baseline.png){#fig-mlp-baseline}

![MLP functions at iteration 21 (best). MLP$_{\text{sub}}$: $\alpha$ at $|s|=1$ is 0.79 — closer to the true scale. MLP$_{\text{node}}$ remains flat, confirming it does not contribute to R² improvement (Principle 7).](figures/rank_50/mlp_best.png){#fig-mlp-best}
