...
![Iteration 21 (best R²). MLP$_{\text{sub}}$: $\alpha$ at $|s|=1$ is 0.79. MLP$_{\text{node}}$: still flat — homeostasis not learned at this configuration.](figures/rank_50/mlp_best.png){#fig-mlp-best}

![Iteration 30 (`sub_norm=2.0`). MLP$_{\text{sub}}$: $\alpha$ at $|s|=1$ is 0.75. MLP$_{\text{node}}$: **active** — learns correct linear homeostasis $-\lambda(c - c^{\text{base}})$. Yet R² = 0.619, lower than iter 21.](figures/rank_50/mlp_active.png){#fig-mlp-active}
...
| 8 (iter 29--32) | L1=0, `sub_norm=2.0`, lr_k | 0.619 | MLP$_{\text{node}}$ active in all configs; `sub_norm=2.0`: fewest outliers (14), best slope (0.99) |
...
7. **MLP$_{\text{node}}$ activates in batch 8 but doesn't improve R²** — learns correct linear homeostasis $-\lambda(c - c^{\text{base}})$, yet R² drops (0.62 vs 0.69)
