...
class LowRankINR(nn.Module):
    ...
    where U (n_frames, rank) and V (rank, n_metabolites) are learnable parameters.
    """

    def __init__(self, n_frames, n_metabolites, rank=64, init_data=None):
        super().__init__()
        ...
        if init_data is not None:
            ...
            self.V = nn.Parameter(torch.randn(rank, n_metabolites) * 0.01)
    ...
    def forward(self, t_indices=None):
        ...
        Returns:
            (batch, n_metabolites) or (n_frames, n_metabolites)
        """
    ...
    def get_compression_ratio(self):
        full_size = self.n_frames * self.n_metabolites
        param_size = self.rank * (self.n_frames + self.n_metabolites)
